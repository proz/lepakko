...
from batsound.live.spectrogram_widget import SpectrogramWidget
from batsound.live.audio_output import AudioOutput
...
class LiveApp(QMainWindow):
    ...
    def _build_ui(self, path: Path) -> None:
        ...
        self.setWindowTitle(f"BatSound Live \u2014 {path.name}")
    ...
    def _compute_chunk_size(self) -> int:
        """Compute chunk size based on method.

        For time-expansion, use smaller chunks so audio playback at sr/division
        stays in sync with the tick rate. Other methods run in real-time.
        """
        if self._method == "time-expansion":
            return self._sr // (self._fps * self._division)
        return self._sr // self._fps

    def _rebuild_reader(self) -> None:
        """Recreate the ChunkReader with the correct chunk size, preserving position."""
    ...
    def _build_method_params(self) -> None:
        ...
        if self._method == "heterodyne":
            ...
            if self._method == "spectral-shift":
                self._params_layout.addWidget(QLabel("Gain:"))
                self._gain_spin = QDoubleSpinBox()
                self._gain_spin.setRange(0.1, 100.0)
                self._gain_spin.setSingleStep(0.5)
                self._gain_spin.setDecimals(1)
                self._gain_spin.setValue(self._gain)
                self._gain_spin.valueChanged.connect(self._on_param_changed)
                self._params_layout.addWidget(self._gain_spin)
    ...
    def _on_param_changed(self) -> None:
        ...
        if self._method == "heterodyne":
            ...
            if self._method == "spectral-shift":
                self._gain = self._gain_spin.value()
    ...
    def _open_file_dialog(self) -> None:
        ...
        path_str, _ = QFileDialog.getOpenFileName(
            self, "Open WAV file", str(self._path.parent), "WAV files (*.wav *.WAV)"
        )
        if path_str:
    ...
    def _tick(self) -> None:
        """Read one chunk, feed to spectrogram and audio."""
